...
Implementation plans are deliberately detailed enough for Luna to execute without guessing: they must include intent/context, non-goals, exact file-level changes, invariants, acceptance criteria, and verification commands with required evidence. The Luna implementation and Sol test-integrity nodes maximize safely independent delegated investigation and verification, while retaining one owner and prohibiting overlapping writes.
