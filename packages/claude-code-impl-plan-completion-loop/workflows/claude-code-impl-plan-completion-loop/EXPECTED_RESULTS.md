...
`claude-code-design-and-implement-review-loop` -> `step3-post-handoff` ->
...
```bash
bun run packages/rielflow/src/bin.ts workflow validate claude-code-impl-plan-completion-loop --workflow-definition-dir .rielflow/workflows
```
...
```bash
bun run packages/rielflow/src/bin.ts workflow run claude-code-impl-plan-completion-loop \
  --workflow-definition-dir .rielflow/workflows \
  --mock-scenario .rielflow/workflows/claude-code-impl-plan-completion-loop/mock-scenario.json \
  --output json
```
...
{
  "status": "completed",
  "workflowName": "claude-code-impl-plan-completion-loop",
  "workflowId": "claude-code-impl-plan-completion-loop",
  "nodeExecutions": 4,
  "transitions": 3,
  "exitCode": 0
}
