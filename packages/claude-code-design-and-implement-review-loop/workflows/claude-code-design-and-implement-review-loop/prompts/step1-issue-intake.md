...
- `runtimeVariables.workflowInput.claudeCodeAgentReferences`
...
- If Claude Code reference planning input is present, inspect the preferred local reference repository first. Use `../../claude-code-agent` when no other local root is supplied. Use the upstream reference URL only if local files are unavailable or incomplete.
- Treat claude-code-agent as a behavioral and structural reference only. Do not copy code blindly.
- Produce one concise intake brief that later steps can execute regardless of mode.
- When the request contains independent feature areas that can be designed and planned concurrently, classify them into `payload.featureFanoutItems`. Each item must include a stable `featureId`, `featureTitle`, `featureSummary`, `issueReference`, `workflowMode`, `designDocPath`, `implPlanPath`, and relevant `claudeCodeAgentReferences`.
...
- `payload.unknowns`
- `payload.risks`
- `payload.claudeCodeAgentReferences`
