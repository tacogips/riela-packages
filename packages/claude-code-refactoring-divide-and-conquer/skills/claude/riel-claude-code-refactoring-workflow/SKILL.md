---
name: riel-claude-code-refactoring-workflow
description: Use when planning, running, monitoring, or troubleshooting the packaged riela divide-and-conquer refactoring workflow. Applies to `claude-code-refactoring-divide-and-conquer`, duplicate-scavenge refactoring, concurrent slice review via `claude-code-refactoring-slice-review`, generated refactoring implementation plans, implementation/review loop handling, blocked verification, dependency-owned agent mocks, and final plan archival.
---

# Riel Claude Code Refactoring Workflow

Use this skill when the user asks to refactor riela through the divide-and-conquer workflow, asks whether the refactoring workflow is progressing, or asks to create/continue a maintainability refactor using workflow review loops.

## Workflow Bundle

- Parent workflow: `claude-code-refactoring-divide-and-conquer`
- Parent package id: `claude-code-refactoring-divide-and-conquer`
- Child workflow: `claude-code-refactoring-slice-review`
- Child package id: `claude-code-refactoring-slice-review`
...
```bash
riela workflow package checkout claude-code-refactoring-divide-and-conquer
riela workflow package checkout claude-code-refactoring-slice-review
riela workflow run claude-code-refactoring-divide-and-conquer \
  --variables '{"workflowInput":{"requestedOutcome":"Improve riela maintainability using divide-and-conquer refactoring.","targetPaths":["src/workflow","src/cli","src/lib.ts","packages",".riela/workflows"],"excludePaths":["dist","node_modules","/tmp","impl-plans/completed"],"maxSlices":6,"constraints":["Do not stage, commit, or push unless the user explicitly asks.","Do not revert unrelated dirty worktree changes.","Keep each implementation pass to one bounded task.","Update any generated refactoring plan progress as tasks are implemented."],"verificationPreferences":["Run narrow Bun tests for touched code first.","Run workflow validate for changed workflow bundles.","Run git diff --check before completion."]}}' \
...
Keep this as a mode of `claude-code-refactoring-divide-and-conquer`; do not create or run a
...
- `claude-code-refactoring-slice-review` owns per-slice duplicate evidence.
...
```json
{
  "workflowInput": {
    ...
    "targetPaths": ["src", "packages", ".riela/workflows"],
...
3. Fan out concurrent child `claude-code-refactoring-slice-review` runs.
...
```bash
bun test src/workflow/adapters/claude-code.test.ts src/workflow/adapters/claude.test.ts
...
```bash
riela workflow validate claude-code-refactoring-divide-and-conquer --output json
riela workflow validate claude-code-refactoring-slice-review --output json
```
...
Do not create riela-local replacements for dependency agent mocks. riela adapter tests should consume dependency-owned test SDK exports, such as:

- `claude-code-agent/sdk/testing`
- `claude-code-agent/sdk/testing`

If those exports are missing, update or fix the owning dependency package (`codex-agent`, `claude-code-agent`, or `cursor-cli-agent`) rather than teaching riela dependency internals.
