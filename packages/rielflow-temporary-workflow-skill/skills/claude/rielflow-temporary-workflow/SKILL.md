---
name: rielflow-temporary-workflow
description: Use when creating, validating, or running temporary rielflow workflows from inline JSON or JSON files without installing a project/user-scope workflow bundle. Applies to --workflow-json, --workflow-json-file, embedded prompt-only payloads, temp workflow payload logs, and dry-run smoke checks.
metadata:
  short-description: Create and run temporary rielflow workflows
---

# Rielflow Temporary Workflow

Use this skill when the user wants a one-off rielflow workflow that can run
