---
name: rielflow-project-workflow
description: Use when requested work should first be converted into a project-scope Rielflow workflow and then executed through Rielflow. Applies to workflow-first implementation, review, refactoring, documentation, or maintenance tasks where no existing package workflow exactly fits, and to requests that explicitly say to work through a project-scope workflow.
metadata:
  short-description: Create and run project-scope Rielflow workflows
---

# Rielflow Project Workflow

Use this skill when the user wants the work shaped into a project-scope
Rielflow workflow before execution, or when the task explicitly requires a
workflow-first process.

## Apply This Skill When

- the user asks to create a workflow for the work before doing the work
- the user says to use a project-scope workflow, `.rielflow/workflows`, or
  Rielflow as the execution path
...
- the user explicitly says not to use Rielflow or not to create a workflow
...
- Use `executionBackend: "codex-agent"` and `model: "gpt-5.5"` for Codex
  agent nodes.
...
```bash
rielflow workflow list --scope project --output json
...
```bash
rielflow workflow validate <workflow-id> --scope project --output json
```

Run the project workflow:

```bash
rielflow workflow run <workflow-id> \
  --scope project \
...
```bash
rielflow workflow validate <workflow-id> --workflow-definition-dir .rielflow/workflows --output json
rielflow workflow run <workflow-id> --workflow-definition-dir .rielflow/workflows --output json
```
...
- workflow id and `.rielflow/workflows/<workflow-id>` path
- validation and run commands with pass/fail results
- session id or artifact location when Rielflow reports one
...
```bash
bun .agents/skills/rielflow-package-release/scripts/update-package-digests.ts rielflow-project-workflow-skill
task check
```
