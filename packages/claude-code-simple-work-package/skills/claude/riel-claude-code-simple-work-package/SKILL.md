...
description: Use when making small code or documentation changes under a directory and no more specialized riela workflow applies. Runs the packaged claude-code-simple-work-package workflow with Claude Code Sonnet 4.5 high-effort implementation and high-effort review, looping back only for high or middle findings.
---
...
```bash
riela workflow run claude-code-simple-work-package \
...
```bash
bun .agents/skills/riela-package-release/scripts/update-package-digests.ts claude-code-simple-work-package
```
