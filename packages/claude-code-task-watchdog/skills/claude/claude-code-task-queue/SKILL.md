...
description: Use when adding, deleting, listing, showing, or editing claude-code task queue items in ./tasks/list.jsonl for the packaged claude-code-task-watchdog workflow.
user-invocable: true
---
...
packaged `claude-code-task-watchdog` workflow.
...
```json
{
  ...
  "prompt": "Concrete work request for Claude Code or a dependency workflow.",
  "workflowInput": {},
  "constraints": [],
  "acceptanceCriteria": [],
...
Kinds:
- `design-and-impl`: dispatches `claude-code-design-and-implement-review-loop`.
- `design-deepthin`, `design-deepthink`, `deepdesign`, or `design`: dispatches `claude-code-deepdesign`.
- `review`: dispatches `claude-code-recent-change-quality-loop`.
- `adhoc`: runs the prompt in the local Claude Code worker step.
...
If a workflow run died and left a stale `running` task, inspect riela
...
After a successful run, `claude-code-task-watchdog` may also create or update
project-scope skills under `.agents/skills` based on the completed Claude Code log and
