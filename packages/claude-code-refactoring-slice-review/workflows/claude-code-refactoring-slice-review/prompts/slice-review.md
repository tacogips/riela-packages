...
Use `when.has_findings: false` and empty `findings` / `proposedTasks` when no actionable high or mid maintainability work exists for the slice. Keep fixture-level examples and stable assertions in `.rielflow/workflows/claude-code-refactoring-slice-review/mock-scenario.json` and `.rielflow/workflows/claude-code-refactoring-slice-review/EXPECTED_RESULTS.md`.
