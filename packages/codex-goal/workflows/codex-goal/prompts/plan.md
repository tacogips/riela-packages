...
- Actively investigate whether the plan should use an available Rielflow workflow before choosing generic local work.
...
- If the accepted goal recommends a more specialized Rielflow workflow, plan the dispatch instead of forcing generic work. Include the workflow id, lookup assumptions, variables, and how this workflow should interpret the delegated result.
- If planning discovers that a different specialized Rielflow workflow is more appropriate than the goal suggested, record the change and why.
