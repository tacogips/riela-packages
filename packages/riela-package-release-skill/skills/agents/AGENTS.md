...
Use this instruction in a `riela-packages` registry checkout when preparing
...
```bash
swift run --package-path "${RIELA_ROOT:-../riela}" riela package search "<package-id>" --output json
```
...
`kind: "node-addon"` packages are included by this script with deterministic
payload digests. Validate node-addon structure through `task package:check-digests`.
