...
const expected = new Map([[codex, 18], [refactor, 6], ['fable-and-improve-codex', 14], ['fable-and-improve-opus', 14]]);
...
let total = 0;
function run(id: string, name: string, mutate: (m: any) => void, verify: (steps: string[]) => void, fixture = 'mock-scenario.json') {
  ...
  const r = spawnSync('riela', args, { encoding: 'utf8' });
...
}, steps => { const i = steps.indexOf('step6-test-integrity-check'); assert.equal(steps[i + 1], 'step6-implement'); });
...
}, steps => { const i = steps.indexOf(`${flavor}-review`); assert.equal(steps[i + 1], `${flavor}-implementation`); assert(!steps.includes('fable-impl-plan')); });
