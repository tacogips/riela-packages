...
Tasks 1–5 are verification/targeted-repair work for existing edits, not fresh implementation. Reuse prior inventories and immutable snapshots, record the fresh baseline and repair only a demonstrated contract mismatch. Task 6 is the remaining primary deliverable.
...
Receipt reuse must retain the original full log, its final CLI outcome and a source-match report under E. When `--happy-receipt` is supplied, assert the happy-path trace and consumed add-on responses from that log and its referenced artifacts; fail clearly on mismatch or missing proof rather than counting the fixture as execution. Other nine cases run with deterministic fixtures. Without that option, run a fresh happy case. Keep source-match evidence tied to hashes of the receipt and actual runtime inputs. The runner must report reused versus newly executed cases separately; both require actual assertions. Do not count nested completion as root success.
...
For receipt reuse, append `--happy-receipt "$PWD/tmp/d4-mock-retry-opus.jsonl"` to the Bun command. Otherwise omit it and record why renewal was necessary. Record existing validation receipt paths and their source comparison; renew validation only when needed. The test runner must journal newly executed mock commands and reused receipt identities with fully expanded paths; each mock needs isolated sessions/artifacts and any knowledge roots. CLI success must match completed session status and assertions. Record expected nonzero negative-case exits as successful assertions only after checking the intended schema diagnostic and absence of consumer execution.
