...
```json
{
  ...
  "acceptanceCriteria": [
    ...
    "All 10 existing regressions plus new default-selection and inherited-route cases pass; every selected scenario has mandatory route assertions and failures propagate; no user registry writes or sibling writes.",
...
4. In `check-package-compat.ts`, retain scenarios mode and `--workflow-list <JSON-array-file>`; omission selects the packaged default cases and must enforce the same route rules as explicit selection. Resolve fixtures through base inheritance before choosing their expected routes. In `.agents/skills/riela-package-release/fixtures/expected-routes.json`, identify entries by effective workflow ID and repository-relative fixture path, with a nonempty ordered step-ID array and graph/fixture/available `EXPECTED_RESULTS.md` source paths. Derive entries from those sources before comparing observed traces; never use actual runs as the sole oracle. Populate the selected default fixtures and inherited selections required by downstream plans. Preserve routing and package fixtures; the map fills missing metadata without copying base nodes.
   Resolve route precedence as explicit selection, fixture metadata, then the effective-workflow/fixture map entry. Reject a supplied malformed value without falling through; reject missing/empty routes, empty/non-string step IDs and IDs absent from the effective graph with workflow/fixture diagnostics. Reuse an inherited base route only when its ordered steps and branch meaning remain valid under effective wrapper patches/replacements; otherwise require an explicit wrapper route. Record route origin, expected route and observed route. Require exact sequence equality, including repeated steps, completed terminal status and existing fixture-backed payload assertions. Keep unique sessions/artifacts and correct fixture path expansion; CLI success or matching payloads alone cannot pass.
   Keep assets mode's existing `--source-root` coverage of skill/frontmatter/reference files, add-on descriptors and documented example syntax against installed CLI help; record exclusions and never run credentialed/live examples. No generalized framework or agent-output reinterpretation.

5. Preserve all 10 existing regression behaviors in `check-package-compat.test.ts`, including workflow inventory drift and explicit wrong-route detection. If stricter fixture prerequisites require fixture updates, retain each original assertion and its intended failing cause. Add default-selection cases for completed/wrong-route/matching-payload failure, the corresponding matching-route pass, and missing/malformed metadata failures. Add inherited effective-wrapper route success and incompatible base-route rejection; assert precedence and no malformed-value fallback. Existing inventory, dependency, nonzero-exit and terminal-state checks must still exercise their original conditions. Use local fixture roots/CLI stubs for harness tests and at least one installed-runtime smoke check. Before releasing dependent workers, reuse hash-matched complete receipts under the rule above or run the checkpointed helper on the untouched baseline source; account for actual legacy failures, missing tool limits, and every component of `mise run check` that is runnable without out-of-tree writes. For original full-check comparison without matching complete receipts, run the intake tree's unchanged `mise run check` in the disposable copy with any required source/build inputs also isolated under tmp; unavailable inputs are a recorded baseline gap, not proof that a new failure is pre-existing. Never run the original sibling-source command against a real sibling checkout.
6. Record configured structural/typecheck commands applicable to changed TypeScript; execute existing checks if present and report their exact argv/results. If no static typecheck is configured, state that limit without inventing a TypeScript project or claiming Bun tests are a typecheck. Deliver baseline.json, coverage.json and commands.json with all failures and complete receipts. Payload validation failures expected at baseline do not block finishing this verification infrastructure plan when the harness tests pass and failures are accurately reported; the specific #117 installed YouTube failure also does not block that acceptance. Unavailable installed CLI or inability to isolate target installation blocks only the dependent behavioral checks that actually require it; report those gaps without manufacturing a running-workflow readiness blocker.
...
The baseline manifest command in the shell block is diagnostic collection, not changed-source acceptance. Reuse matching complete receipts or run both `baselineDiagnostics` commands above. Preserve its nonzero result and complete log in diagnostic evidence, not in the Step 6 `verification` array.
